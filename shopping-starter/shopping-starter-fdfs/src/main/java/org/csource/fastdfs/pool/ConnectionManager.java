...
public class ConnectionManager {
    ...
    public Connection getConnection() throws FdfsException {
        lock.lock();
        try {
            ...
            while (true) {
                if (freeCount.get() > 0) {
                    ...
                    try {
                        ...
                        throw new FdfsException("connect to server " + inetSocketAddress.getAddress().getHostAddress() + ":" + inetSocketAddress.getPort() + " fail, wait_time > " + ClientGlobal.g_connection_pool_max_wait_time_in_ms + "ms");
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                        throw new FdfsException("connect to server " + inetSocketAddress.getAddress().getHostAddress() + ":" + inetSocketAddress.getPort() + " fail, emsg:" + e.getMessage());
                    }
                }
                return connection;
            }
